...
class Mdepartment extends CI_Model{
    ...
    public function addMember(){
        ...
        $username = $this->Pinyin($input['username'],1);
        $mobile = $input['tel'];
        $uiserid = $username.$mobile;
        $this->load->model('Enterprise_WeChat/Token','Token');
        $secret = "Vene8y-VCLinsPOFCm6nRBeW-rVyhJAJG6btR3lo81s";
        $TokenName = "Mail_list_Token";
        ...
        $options=[
            ...
            "name"          =>$input['username'],
            'mobile'        =>$mobile,
            'department'    =>$input['company_group'],
        ];
          $result=$this->curl_post_https($url,json_encode($options,JSON_UNESCAPED_UNICODE));
          $errcode = json_decode ($result,true)['errcode'];
          if($errcode === 0){
                $data = [
                   'wc_userid' =>$uiserid
                ];
                $rs = $this->db->update('company_member',$data,['tel'=>$mobile]);
          }
          return $result;
    }  
